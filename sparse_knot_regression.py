...
#ridge regression
def RR(X, y, lam=1, centering=False, standarize=False):
    """
    Minimize in closed form the error function embedded in a sum-of-squares error
    term. These are shrinkage methods or weight decay methods. Apply RR and return
    this weight vector.
    """
 ...
 #active learning
 def AL(X_train, X_test, lam=1, sigma2=1):
     """
        ...
        for i in xrange(10):
            ...
            for idx in range (len(D)):
                if idx not in selected:
                    ...
                    sigma2 = sigma2 + np.dot(np.transpose(x0), np.dot(cov, x0))


            #Steepest sigma descent. Find Maxima.
 ...
 if __name__ == '__main__':
     ...
     X_train = np.genfromtxt(sys.argv[4], delimiter=',', skip_header=0, names=None)
     X_test = np.genfromtxt(sys.argv[5], delimiter=',', skip_header=0, names=None)
     wRR = RR(X_train, y_train, lam)
     active = AL(X_train, X_test, lam, sigma2)
